...
<div class="row" >
   ...
   <div class="col-sm-12">
        <div class="hr_divider" style="text-align: center">
            <table align="center" class="table" style="max-width: 500px; text-align: left;margin-bottom:0px;">
                <tr>
                    <td colspan="3" class="alert alert-warning"> <?php $U = $this->plan_model->create_tree($top_id);
                                                                    echo $U['data'] ?></td>
                </tr>
            </table>
            <div class="table-responsive" style="overflow-x: auto; text-align: left; overflow-y: auto;height: 380px;">
                <table align="center" class="table" style="max-width: 500px">
                    <?php

                    $this->db->select('id, name, sponsor, status,total_a, mypv, topup, total_a_pv, my_img')->where('position', $top_id);
                    $data = $this->db->get('member')->result();

                    foreach ($data as $e) {
                        $spon = $this->db_model->count_all('member', array('position' => $e->id));
                        if ($e->topup == "0.00") {
                            $color = 'grey';
                        } else {
                            if ($e->status == "Block") {
                        ...
                        $colors = ($color == 'yellow') ? "black" : $color;
                        echo '
                    <tr>
                        <td></td>
                        <td></td>
                        <td style="border-left: 4px dashed #006aeb;">
                            ...
                            <span style="text-align: center">
                                <a href="' . site_url('tree/user_tree/' . $e->id) . '" style="text-decoration: none; color: ' . $colors . '; margin: 5px" data-toggle="popover" data-trigger="hover" data-html="true" data-placement="top" title="<div align=\'left\'><strong>' . config_item('ID_EXT') . $e->id . '</strong><hr/>Total Downline:' . ($spon) . '<br/>Sponsor: ' . ($e->sponsor) . '</div>">
        ...
        <div class="table-responsive">
            <table class="table table-borderless">
                <tbody>
                    ...
                    <tr>
                        <td colspan="2" align="center"><?php $A_1 = $this->plan_model->create_tree($A['A'], $A['id'], 'A');
                                                        echo $A_1['data'] ?><br /><img src="<?php echo base_url('uploads/site_img/line_sm.gif') ?>" class="img-responsive">
                        </td>
                        <td colspan="2" align="center"><?php $A_2 = $this->plan_model->create_tree($A['B'], $A['id'], 'B');
                                                        echo $A_2['data'] ?><br /><img src="<?php echo base_url('uploads/site_img/line_sm.gif') ?>" class="img-responsive">
                        </td>
                        <td colspan="2" align="center"><?php $B_1 = $this->plan_model->create_tree($B['A'], $B['id'], 'A');
                                                        echo $B_1['data'] ?><br /><img src="<?php echo base_url('uploads/site_img/line_sm.gif') ?>" class="img-responsive">
                        </td>
                        <td colspan="2" align="center"><?php $B_2 = $this->plan_model->create_tree($B['B'], $B['id'], 'B');
                                                        echo $B_2['data'] ?><br /><img src="<?php echo base_url('uploads/site_img/line_sm.gif') ?>" class="img-responsive">
                        </td>
                    </tr>
                    <tr>
