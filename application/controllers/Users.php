...
class Users extends CI_Controller
{
    ...
    public function topup_member(){          
        ...
        $this->form_validation->set_rules('amt', 'Total Amount', 'trim|required');
        $this->form_validation->set_rules('invested_in', 'Investd In', 'trim|required');
        $this->form_validation->set_rules('qty', 'Share Quantity ', 'trim|required');
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $invested_in   = $this->input->post('invested_in');
    ...
    public function account_close()
    {
        ...
        // $this->db->select('id, name, phone, sponsor, topup, join_time, total_a, total_b, total_c, total_d, total_e')
        //          ->from('member')->where(array('status' => 'Account_close'));

                 $this->db->select("m.name,m.phone,m.status as account_status,ac.*")
       ->where('m.status','Account_close')->from('member as m')
       ->join('account_closed as ac','ac.member_id=m.id');
        ...
        $data['breadcrumb'] = 'Account Closed';
        $data['layout']     = 'member/account_close.php';
        $this->load->view('admin/base', $data);

    }
    function close_account_payment($id)
    {
        $dat=array(
            'investment_refund_date' =>date('Y-m-d'),
            'status' =>'Paid',
        );
        $this->db->where('id',$id)->update('account_closed',$dat);
        redirect('Users/account_close','refresh');
