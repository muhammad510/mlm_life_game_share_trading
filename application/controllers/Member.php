 ...
 */
    ...
    function upload_profile_image()
    {
        ...
        $dat=$this->upload_image('member_image','pic'); 
        if($dat['icon']=='error')
        {
            $this->session->set_flashdata("common_flash", "<div class='alert alert-danger'>".$dat['text']."</div>");
            redirect('Member/profile_image', 'refresh');
        }
        else{ 
            $mem=$this->db->select()->where('id',$this->session->user_id)->get('member')->row_array();

            if(file_exists($mem['photo']))
            {
                unlink($mem['photo']);
            }
       ...
       $a= $this->db->where('id',$this->session->user_id)->update('member',$data);

        if ($a) {
            $this->session->set_flashdata("common_flash", "<div class='alert alert-success'>Profile Picture Updated successfully </div>");
    ...
    function profile_image()
    {
            ...
            $data['title'] = 'Change Profile Image';
            $data['layout'] = 'profile/profile_image.php';
            $this->load->view('member/base', $data);       
    ...
    function holding()
    {
        $data['share'] = $this->db->select('*')->from('topup_record')->where('user_id', $this->session->user_id)->get()->result_array();

        $data['title'] = 'My Holding';
        $data['layout'] = 'holding/holding.php';
        $this->load->view('member/base', $data);
    }

    function print_invest()
    {
        $id = $this->input->post('id');
        $con = array('id' => $id, 'user_id' => $this->session->user_id);
        $data['share'] = $this->db->select('*')->from('topup_record')->where('id', $id)->where('user_id', $this->session->user_id)->get()->row_array(); 

        $data['member'] = $this->db->select('*')->from('member')->where('id', $this->session->user_id)->get()->row_array();
          
        $this->load->view('member/holding/print', $data);
    }
    function print_invest_all()
    {
        ...
        $this->load->view('member/holding/print_all', $data);
    ...
    function invest()
    {
        ...
        if ($this->form_validation->run() == FALSE) {
            $data['title']      = 'Invest Again';
            $data['breadcrumb'] = 'Invest Again';
            $data['layout']     = 'holding/invest.php';
            $data['products']   = $this->db->get('product')->result_array();
            $data['epin']      = $this->db->select('epin,amount')->Where(array('issue_to' => $this->session->user_id, 'status' => 'Un-used'))->get('epin')->result_array();
            $this->load->view('member/base', $data);
        } else {
            ...
            if ($epin_value > 0) {
                ...
                $data = array(
                    ...
                    'epin' => trim($this->input->post('topup')),
                    'invested_in' => trim($this->input->post('invested_in')),
                    'share_quantity' => trim($this->input->post('quantity')),
                    'discount' => trim($this->input->post('discount')),
                    'paybale_amount' => trim($this->input->post('paybale_amount')),
                ...
                redirect(site_url('Member/holding'));
            } else {
    ...
    function get_epin_value()
    {


        $epinid = $this->input->post('epinid');
        $amount = $this->db->select('amount')->from('epin')->where('epin', $epinid)->get()->row_array();
    ...
    public function topup()
    {
        ...
        if ($epin_value > 0) {
            ...
            $data = array(
                ...
                'epin' => trim($this->input->post('topup')),
                'invested_in' => trim($this->input->post('invested_in')),
                'share_quantity' => $epin_value / 1000,
                'topup_amount' => $epin_value,
                'topup_by' => $this->session->user_id,
                'date' => date('Y-m-d'),
            );
    ...
    function close_account()
    {
        ...
        $data['close_info'] = $this->Close_model->get_close_ac($this->session->user_id);
        if ($data['close_info'] != '') {
            $data['title'] = 'Account Closed';
            $data['layout'] = 'account_close/print.php';
        } else {
            $data['title'] = 'Account Closing Term And Condition';
            $data['layout'] = 'account_close/close_tc.php';
        }
    ...
    {
        $amount = $this->db->select_sum('topup_amount')->from('topup_record')->where('user_id', $this->session->user_id)->get()->result_array();
        $data['amount'] = $amount[0]['topup_amount'];
        ...
        $data = array(
            ...
            'total_investment' => $amount[0]['topup_amount'],
            'account_closing_date' => date('Y-m-d'),
            'investment_refund_date' => date('Y-m-d', strtotime(date('Y-m-d') . '+' . config_item('refund_days') . 'days')),

        );
        $this->db->insert('account_closed', $data);
        $data = array(
            'status' => 'Account_close'
        );
        $this->db->where('id', $this->session->user_id)->update('member', $data);

        $this->load->model('Close_model');
        $data['close_info'] = $this->Close_model->get_close_ac($this->session->user_id);

        $data['layout'] = 'account_close/print.php';
        $this->load->view('member/account_close/print', $data);
    }


    function check_closed_account()
    {
        $this->load->model('Close_model');
        $data['close_info'] = $this->Close_model->get_close_ac($this->session->user_id);
        if ($data['close_info']) {
            $data['layout'] = 'account_close/print.php';
            $page = $this->load->view('member/account_close/print', $data);

            $data = array('text' => $page, 'icon' => 'success');
        }
